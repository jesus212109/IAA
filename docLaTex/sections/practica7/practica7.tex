...
\begin{table}[H]
    \centering
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lccp{4cm}p{4.5cm}}
    \toprule
    \textbf{Modelo} & \textbf{Acc. Test} & \textbf{Coste} & \textbf{Ventajas} & \textbf{Desventajas} \\ \midrule
    Árbol Simple      & 0.923 & Bajo  & Interpretable, ultrarrápido, poca memoria. & Se sobreajusta con facilidad. \\
    Random Forest     & 0.958 & Medio & Robusto, reduce varianza, paralelizable.    & Difícil de interpretar (caja negra). \\
    Gradient Boosting & 0.958 & Alto  & Alta precisión, corrige errores paso a paso. & Secuencial, sensible a hiperparámetros. \\ \bottomrule
    \end{tabular}}
...
Esta práctica ha demostrado que los modelos de ensamble mejoran significativamente el rendimiento predictivo respecto a un solo árbol de decisión. El Árbol Simple, pese a su interpretabilidad y bajo coste, presenta un sobreajuste severo (accuracy en test de 0.923 frente a 1.000 en entrenamiento). Random Forest mitiga este problema mediante bagging y promediado de predicciones, alcanzando un 0.958 de accuracy con 100 árboles y estabilizándose a partir de 50 estimadores.

Gradient Boosting iguala el rendimiento de Random Forest (0.958), pero con un enfoque secuencial que corrige errores residuales. Aunque computacionalmente más costoso, destaca por su alta precisión. El análisis de importancia de variables revela que \texttt{worst radius} (55.9\,\%), \texttt{worst concave points} (13.2\,\%) y \texttt{worst perimeter} (12.8\,\%) son los predictores más relevantes, coherente con la caracterización de núcleos celulares malignos.

La principal limitación de los ensambles es la pérdida de interpretabilidad y el mayor coste computacional, lo que condiciona su despliegue en entornos con recursos limitados. Si tuviese que usar uno de estos modelos en un dispositivo con poca capacidad de proceso, creo que usaría el Árbol Simple o un Random Forest muy pequeño (con 1-10 árboles). Se perdería algo de precisión (del 95.8\,\% al 92.3\,\%), pero el modelo ocuparía muy poco y haría predicciones casi instantáneas.
